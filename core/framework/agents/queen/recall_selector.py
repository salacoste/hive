...
def _extract_json_payload(raw: str) -> Any:
    ...
    The selector requests ``json_object`` mode, but some providers can still
    wrap payloads with markdown fences or brief prefixed/suffixed text.
    """
    text = (raw or "").strip()
    if not text:
    ...
    # 3) First JSON-looking object/array slice from the text.
