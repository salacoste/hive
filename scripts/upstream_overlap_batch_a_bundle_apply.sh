...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/upstream_overlap_batch_a_bundle_apply.sh [--check|--apply]
...
for path in "${REPLAY_MANIFEST}" "${DEP_MANIFEST}" "${HOTSPOTS_MANIFEST}"; do
  if [[ ! -f "${path}" ]]; then
...
echo "== Overlap Batch A Bundle Apply =="
echo "mode=${MODE}"
