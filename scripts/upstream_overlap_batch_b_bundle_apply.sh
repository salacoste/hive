...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/upstream_overlap_batch_b_bundle_apply.sh [--check|--apply]
...
for path in "${REPLAY_MANIFEST}" "${DEP_MANIFEST}" "${FRONTEND_MANIFEST}"; do
  if [[ ! -f "${path}" ]]; then
...
for bundle in "${REPLAY_BUNDLE}" "${DEP_BUNDLE}" "${FRONTEND_BUNDLE}"; do
...
echo "== Overlap Batch B Bundle Apply =="
echo "mode=${MODE}"
